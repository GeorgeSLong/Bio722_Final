...
merged_physeq <- merge_samples(physeq_asinh, "Treatment") # Pooling the treatments
...
for(i in colnames(kraken_taxa_df)){
	...
	pdf(paste0("../Graphs/Asinh_Heatmap_Not_NULL",i,".pdf"), width = 9, height = 6)
		print(plot_heatmap(Glommed, method = "PCoA") + ylab(paste(i)))
	dev.off()
}
...
for(i in distances_tested){
	Distance_Testing[[count]] <- plot_ordination(physeq_asinh, ordinate(physeq_asinh, "PCoA", i), type = "samples", color = "Treatment") + geom_point(size = 1) + theme(legend.position="none") + ggtitle(i)
	count <- count + 1
}
pdf("../Graphs/Distance_Testing_Asinh.pdf", height = 40, width = 24)
	grid.arrange(grobs = Distance_Testing, ncol = 4)
dev.off()

# Actually making the plot
GP.ord <- ordinate(physeq_asinh, "PCoA", "bray")
for(i in colnames(physeq_asinh@sam_data)){
	pdf(paste0("../Graphs/PCoA_",i,"_asinh.pdf"), width = 6, height = 4)
...
for(i in colnames(physeq_asinh@sam_data)){
	pdf(paste0("../Graphs/Network_",i,"_asinh.pdf"), width = 6, height = 4)
		set.seed(1996)
...
pdf("../Graphs/WalkTrap_asinh.pdf", width = 6, height = 6)
	plot(nit,ig, layout = layout, vertex.label = Treatments$Treatment, vertex.label.color = "black",  vertex.size = 10, edge.arrow.size = .2)
dev.off()
...
pdf("../Graphs/Permutation_Compost_asinh.pdf", width = 6, height = 4)
...
# Graphing the Results
pdf("../Graphs/Permutation_Nitrogen_asinh.pdf", width = 6, height = 4)
...
# Permutation Test - Phosphorus
perm_pho  <- replicate(n = 10000,{
	 mem <- sample(x = num, size = 9, rep = FALSE)
	 mem_num <- sapply(colnames(physeq_prop@otu_table), function(x){ifelse(x %in% mem, 1,2)})
	 return(modularity(make_clusters(ig, membership = mem_num)))
})
(sum(perm_pho > modularity(pho)) + 1)/ (length(perm_pho) + 1)

# Permutation Test - Potassium
perm_pot  <- replicate(n = 10000,{
	 mem <- sample(x = num, size = 9, rep = FALSE)
	 mem_num <- sapply(colnames(physeq_prop@otu_table), function(x){ifelse(x %in% mem, 1,2)})
	 return(modularity(make_clusters(ig, membership = mem_num)))
})
(sum(perm_pot > modularity(pot)) + 1)/ (length(perm_pot) + 1)
...
shannon_plot <- ggplot(alpha_div, aes(y = Shannon, x = Treatment)) + geom_boxplot() + geom_text(data = shannon_df, aes(label = Letters)) + ggtitle("Shannon Diversity - asinh Transformation") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
simpson_plot <- ggplot(alpha_div, aes(y = Simpson, x = Treatment)) + geom_boxplot() + geom_text(data = simpson_df, aes(label = Letters))+ ggtitle("Simpson Diversity - asinh Transformation")+ theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Let's plot these results
pdf("../Graphs/Diversity_Analysis_asinh.pdf", width = 6, height = 8)
